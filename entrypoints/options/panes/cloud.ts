...
export async function renderCloudPane(
  ...
  // rows. The resolved adapter is reused for the display read and the wipe below.
  ...
  const status = await readCloudStatus(() => Promise.resolve(adapter));
  ...
  let currentPendingCount = pendingCount;

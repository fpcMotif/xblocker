...
//
// The tweet-article selector here was verified against a live zh-Hant x.com DOM (2026-06),
// in the same session as the User-Name byline and socialContext selectors — but those two
// stay module-private in author.ts (their only consumer): this file hosts the vocabulary
// matched across surfaces, not every X selector. The confirmationSheetConfirm testid is
// reused by X across many sheets (block, delete, unfollow, log out) and localizes its text,
// so the testid alone can never classify a sheet — that is why the block/mute menu LABELS
// live here beside it, keyed off the locale-stable menu-item testids first and the visible

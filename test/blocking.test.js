...
describe("Blocking and Muting Functions", () => {
  ...
  beforeEach(() => {
    ...
    global.setTimeout = mock((fn, _delay) => {
  ...
  describe("Page State Validation", () => {
    test("should validate tweet page URL", () => {
      const checkPageType = (url) => {
        const tweetPagePattern = new RegExp(String.raw`https?://(www\.)?x\.com/[^/]+/status/\d+`);
        const timelinePattern = new RegExp(String.raw`https?://(www\.)?x\.com/i/timeline`);

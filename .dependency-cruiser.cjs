...
module.exports = {
  forbidden: [
    ...
    },
    {
      name: "no-circular",
      comment:
        "No dependency cycles. Type-only and lazy dynamic-import edges are exempt: they carry no module-init order, and the repo deliberately closes loops with them (sync-engine lazy-loads convex-sync, which imports back only a type).",
      severity: "error",
      from: {},
      to: { circular: true, dependencyTypesNot: ["type-only", "dynamic-import"] },
    },
  ...
  options: {
    ...
    // See imports as written, before TS compilation elides unused and
    // type-only ones — a type-only deep import still breaks the boundary.
